...
**存储卷 (Volume)**:
一个采集 generation 所属的机身可移除存储身份。
_Avoid_: 手机存储、会话目录

**安全换盘回执 (Safe-swap Receipt)**:
由机身签发并绑定身份的证明，表示特定 generation、会话和存储卷已经封存、释放且没有打开句柄。
_Avoid_: 已停止录制、待机、可以猜测的状态

**网络切换事务 (Network Transition)**:
由机身拥有的网络配置应用、重试或忘记尝试，同时保留明确的救援路径。
